  ...
  const commands = useMemo<Command[]>(() => {
    ...
    const actionCommands: Command[] = [
      ...
      ...(projectId ? [
        {
          id: 'action-voice-dna',
          label: 'Analyze Voice DNA',
          description: 'Build voice fingerprints for characters',
          icon: Mic,
          category: 'action' as const,
          keywords: ['voice', 'character', 'dialogue', 'fingerprint', 'speech', 'pattern'],
          action: () => { navigate(`/projects/${projectId}/characters?view=voiceDna`); onClose() },
        },
        {
          id: 'action-subplots',
          label: 'Manage Subplots',
          description: 'Track subplot threads and tension curves',
          icon: GitBranch,
          category: 'action' as const,
          keywords: ['subplot', 'thread', 'tension', 'arc', 'storyline'],
          action: () => { navigate(`/projects/${projectId}/plot?view=subplots`); onClose() },
        },
        {
          id: 'action-reader-sim',
          label: 'Simulate Reader Experience',
          description: 'Predict reader emotional journey',
          icon: Eye,
          category: 'action' as const,
          keywords: ['reader', 'emotion', 'experience', 'predict', 'simulation'],
          action: () => { navigate(`/projects/${projectId}/review?tab=reader`); onClose() },
        },
        {
          id: 'action-continuity',
          label: 'Check Continuity',
          description: 'Find and resolve continuity conflicts',
          icon: AlertTriangle,
          category: 'action' as const,
          keywords: ['continuity', 'conflict', 'consistency', 'facts', 'error'],
          action: () => { navigate(`/projects/${projectId}/review?tab=continuity`); onClose() },
        },
        {
          id: 'action-guided-write',
          label: 'AI-Assisted Writing',
          description: 'Generate scenes with approval workflow',
          icon: Sparkles,
          category: 'action' as const,
          keywords: ['generate', 'ai', 'write', 'assist', 'guided', 'auto'],
          action: () => { navigate(`/projects/${projectId}/write?mode=guided`); onClose() },
        },
        {
          ...
          icon: Lightbulb,
          category: 'action' as const,
          keywords: ['ideas', 'brainstorm', 'creative', 'generate'],
          action: () => { navigate(`/projects/${projectId}/brainstorm`); onClose() },
        },
        {
          ...
          icon: UserPlus,
          category: 'action' as const,
          keywords: ['character', 'create', 'add', 'new', 'person'],
          shortcut: '⇧C',
          action: () => { navigate(`/projects/${projectId}/characters?action=new`); onClose() },
        },
        {
          id: 'action-new-scene',
          label: 'New Scene',
          ...
          icon: Plus,
          category: 'action' as const,
          keywords: ['scene', 'create', 'add', 'new'],
          shortcut: '⇧S',
          action: () => { navigate(`/projects/${projectId}/scenes?action=new`); onClose() },
        },
      ] : []),
      {
